fit.mix.par.dedup.Dist <-
function(D.mat, match.constr = c("N:M", "1:1"),
...
# two components inflated-beta mixture (a DEDUPLICATION task is   #
# assumed):                                                       #
#                                                                 #
...
#       Either Many-to-Many and One-to-One matching restrictions  #
#       can be handled. One-to-One constraints are meaningful if  #
#       it is known in advance that the file to be deduplicated   #
#       was actually built by appending two duplicate-free        #
#       sources which can no more be identified.                  #
...
# Check files dimensions and retrieve lower triangle values
if ( ( m <- nrow(D.mat) ) != ( n <- ncol(D.mat) ) )
    stop("Deduplication needs a square Distance Matrix")
T.mat <- D.mat[row(D.mat) > col(D.mat)]
npairs <- length(T.mat)
...
#  (i) T.mat contains only values 0 and 1     (vary rare cases)
# (ii) trimmed version of T.mat is non-empty  (almost always)
...
n.0 <- sum(T.mat <= zero)
n.1 <- sum(T.mat >= one)
T.trim <- T.mat[ ( T.mat > zero ) & ( T.mat < one) ]
npairs.trim <- length(T.trim)
...
n.M.max <- switch(match.constr,
                  "1:1" = if (n.0 <= floor(m/2)) floor(m/2) else {warning(warn.msg, immediate. = TRUE); (npairs - n.1)},
                  "N:M" = npairs - n.1)
...
au <- par[1]
bu <- par[2]
log.p <- dbeta(T.trim,au,bu,log=TRUE)
-sum(log.p)
}
...
am <- par[2]
bm <- par[3]
p <- pm*dbeta(T.trim,am,bm)+(1-pm)*f.u
-sum(log(p))
}
      ...
      if ( force && (step2.out$message %in% c("iteration limit reached without convergence (9)",
          ...
          warning("STEP2 limit reached", immediate. = TRUE)
           ...
           if ( !(step2.out$message %in% c("absolute function convergence (6)",
               ...
               print(optim.out)
               stop("STEP2 optimization failed: try different starting values")
               }
           }
    }
...
# F T
      if (match.constr=="N:M") {
          solution <- "MAPs"
          cls <- c("dedup.mix.par.sol.NtoM", "dedup.mix.par.sol")
        }
# F F
      if (!a.has.dup && !b.has.dup) {
          if (match.constr=="1:1") {
              ...
              cls <- c("dedup.mix.par.sol.1to1", "dedup.mix.par.sol")
            }
        }

    EC.M <- nrow(MAP.l)
  # Unconditional expected number of Matches given files (i.e. T.mat) dimensions
...
match.c <- switch(match.constr,
                  "1:1" = "1to1",
                  "N:M" = "NtoM")
class(out) <- c(paste("dedup.mix.par", match.c, sep="."), "dedup.mix.par")
...
plot.dedup.mix.par <- function(mix.par, D.mat){
###########################################
...
# model fit.                              # 
###########################################
if (!inherits(mix.par,"dedup.mix.par")) 
...
if ( isTRUE(all.equal((Dmin <- min(T.mat)),(Dmax <- max(T.mat)))) ){
...
plot1 <- expression(
{
h <- hist(T.mat, breaks = breaks, plot = FALSE) 
...
############################################
# Print methods for class "dedup.mix.par"  #
############################################
print.dedup.mix.par <- function(x, ...){
cat("Fitted inflated-beta mixture model for Deduplication\n")
...
print.dedup.mix.par.sol.1to1 <- function(x, ...){
cat("* 1:1 RL solution found *\n")
print.dedup.mix.par(x,...)
}
